...
async function loadCategories() {
    ...
    try {
        ...
        if (!data.success) {
            showCategoryMessage("Failed to load categories.");
            return;
        }

        if (data.categories.length === 0) {
            showCategoryMessage("No categories found.");
            return;
        }
...
async function loadDashboard() {
    ...
    try {
        ...
        document.getElementById(
            ...
            `Rs. ${parseFloat(summary.total_income).toFixed(2)}`;

        document.getElementById(
            "totalExpenses"
        ).textContent =
            `Rs. ${parseFloat(summary.total_expenses).toFixed(2)}`;
